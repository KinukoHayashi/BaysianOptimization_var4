...
def MovAveZ(s, test_num):
    ...
    for i in range(10, 500, 5):
        ...
        # outputfile = "D:/hayashi_dem/drum_dem_test" + \str(test_num)+"/test"+str(test_num) + \"_" + str(s)+"_data/"+str(i)+".h5"
        outputfile = "D:/hayashi_dem_GB/GB_drum_dem_test" + \
            str(test_num)+"/test"+str(test_num) + \
            "_" + str(s)+"_data/"+str(i)+".h5"
        ...
        if (i % 50 == 0 and i >= 150 and flag == 0):
            ...
            for d in range(1, 5):
                if (moving_avarage[d][len(moving_avarage[d])-1] != None and moving_avarage[d][len(moving_avarage[d])-1-10] != None):
                    grad = ((moving_avarage[d][len(
                        moving_avarage[d])-1]-moving_avarage[d][len(moving_avarage[d])-1-10])/2)
