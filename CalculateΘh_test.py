...
#def Calculateθh(particle_radius,safetime):
safetime = 2
particle_radius = 0.002975
degree = 0
hight = 0
...
count = 1
for i in range (safetime*100,(safetime*100)+301,100): #3s,4s,5s,6sのさんぷりんぐ
    PositionX_0_1 = []
    PositionY_0_1 = []
    PositionZ_0_1 = []
    PositionX_0_2 = []
    ...
    outputfile = "D:/hayashi_dem/drum_dem_rpm22/rpm22_data/"+str(i)+".h5"
   
    #"D:/hayashi_dem/drum_dem _test1/test2_data/"+str(i)+".h5"
    ...
    H = PositionZ_0_temp[len(PositionZ_0_temp)-1]-PositionZ_0_temp[0] +particle_radius*2 #m
    print(str(H)+"mm")
    ...
    print(str(count)+"回目の試行結果"+str(SAMPLE)+"個サンプリングしたときの動的安息角は"+str(θ)+"°です。高さは"+str(H*1000)+"mmです.")
    degree = degree + θ
    hight = hight + H*1000 #mm
    count = count + 1

print(str(count-1)+"回の試行結果の平均は"+str(degree/(count-1))+"°です。高さは"+str(hight/(count-1))+"mmです.")
